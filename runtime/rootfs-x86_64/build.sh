...
image="${VODKA_ROOTFS_IMAGE:-vodka-rootfs-x86_64:bookworm}"
engine="${VODKA_ENGINE:-docker}"
platform="linux/amd64"
archive="rootfs-x86_64.tar.gz"

usage() {
  cat <<EOF
Build the Vodka x86-64 Wine rootfs (FEX guest rootfs).

Environment:
  VODKA_ENGINE         container engine (default: docker; podman also works)
  VODKA_ROOTFS_OUT     output directory (default: build/rootfs)
  VODKA_ROOTFS_IMAGE   image tag (default: vodka-rootfs-x86_64:bookworm)
  VODKA_DXVK_VERSION   DXVK release to stage (build arg)
